	...
	private static void predict(BufferedReader input, DataOutputStream output, svm_model model, int predict_probability) throws IOException
	{
		...
		if(predict_probability == 1)
		{
			...
			else
			{
				...
				output.writeBytes("predicted_label\ttarget_label\t");
				for(int j=0;j<nr_class;j++)
					output.writeBytes("probability_label_"+labels[j]+"\t");
				output.writeBytes("\n");
			}
		}
		while(true)
		{
			...
			else
			{
				...
				tst=svm.svm_predict2(model,x);				
				output.writeBytes(tst[0] +"\t" +tst[1]+"\t" +target +"\n");
//				output.writeBytes(v+"\n");
				v=tst[0];
			}
